...
class AdrDocService(DocService):
    ...
    def get_last_edited_goods(self, to_json=False):
        query_docs = f'SELECT * FROM RS_adr_docs WHERE id_doc = ? and verified = 1'

        query_goods = '''
        SELECT * FROM RS_adr_docs_table
    ...
    def update_sent_data(self, data):
        if data:
            for doc in data:
                id_doc = doc['id_doc']
                id_goods = ','.join([f'"{item.get("id_good")}"' for item in doc.get('RS_adr_docs_table', [])])
    ...
    def get_doc_value(self, key, id_doc):
        query = f'SELECT {key} from RS_adr_docs  WHERE id_doc = ?'
        res = self._get_query_result(query, (id_doc,), True)
        if res:
            return res[0][key]

    def get_doc_types(self) -> list:
        query = 'SELECT DISTINCT doc_type from RS_adr_docs'
        doc_types = [rec[0] for rec in self._get_query_result(query)]
        return doc_types
