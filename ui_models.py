        ...
        if not data_for_printing:
            ...
            if str_table_view:  # Нашли настройки для вызвавшего экрана
                ...
                htmlresult = printing_factory.HTMLDocument(template_directory, template_file).create_html(
                    data_for_printing, params_match.get('barcode_type'))
...
class DocDetailsScreen(Screen):
    ...
    def _barcode_scanned(self):
        ...
        if res is None:
            ...
            if res['Error'] == 'AlreadyScanned':
                ...
                self.open_series_screen(id_doc, res['current_elem'])
    ...
    def open_series_screen(self, id_doc, current_elem):
        ...
        params_for_series_screen = json.dumps(current_elem)
...
class GoodsBalancesItemCard(Screen):
    ...
    def _reset_balances_tables(self):
        vars_list = ['wh_select', 'input_item_id', 'cell_input', 'cell_name', 'object_name', 'error_msg',
                     ...
                     'selected_wh_id', 'barcode_info', 'selected_wh_name', 'selected_cell_name']
    ...
    def _prepare_table_data(self, raw_balances_data):
        ...
        for el in raw_balances_data:
            table_row = {'key': str(el['id_good']),
                         ...
                         'storage_name': str(el['name_warehouse']) if self.table_type == 'warehouses' else str(
                             el['name_cell']),
                         'qtty': str(el['qtty']),
...
class SeriesList(Screen):
    ...
    def __init__(self, hash_map: HashMap, rs_settings):
        ...
        if self.params.get('warehouse'):
            self.params['id_warehouse'] = self.service.get_item_by_name(self.params['warehouse'], 'RS_warehouses')
...
class SeriesItem(SeriesList):
    ...
    def on_start(self):
        prop_list = self.service.get_series_prop_by_id(self.hash_map.get('current_series_id'))
